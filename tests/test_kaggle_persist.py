...
— see that module's docstring and tests/kaggle_api_fake.py's). The fake
replicates the one behavior this module's correctness actually depends on:
dataset_create_version() is a full-snapshot REPLACE, not a diff — whatever
isn't in the uploaded folder disappears from the new version. That's
exactly what push_checkpoint()'s pull-then-merge-then-push design exists to
work around, and test_pushing_one_model_never_deletes_another_already_persisted_model
below is the test that would catch a regression of that design.
"""
